...
def can_match(a, b):
	return (a == b) or (a == "5" and b == "s") or (a == "s" and b == "5")
...
while(True):
	...
	if (pyautogui.pixelMatchesColor(int(minLeft - TURN_X_DELTA), int(minTop - TURN_Y_DELTA), (250, 218, 38), tolerance=50)):
			...
			for x in range(1, GRID_WIDTH):
				if (grid[y][x] != "?"): # brute force a detect 3 move, inspired by https://github.com/akleemans/bejeweled-bot/blob/master/bejeweled-bot.py
					...
					if (can_match(grid[y][x], grid[y][x-1])):
						if (in_bounds(x+1, y-1) and can_match(grid[y][x], grid[y-1][x+1])):
						...
						if (in_bounds(x-2, y-1) and can_match(grid[y][x], grid[y-1][x-2])):
							moves[(x-2, y-1, x-2, y)].append(grid[y][x])
						if (in_bounds(x-3, y) and can_match(grid[y][x], grid[y][x-3])):
							moves[(x-3, y, x-2, y)].append(grid[y][x])
						if (in_bounds(x-2, y+1) and can_match(grid[y][x], grid[y+1][x-2])):
							moves[(x-2, y+1, x-2, y)].append(grid[y][x])

					if (can_match(grid[y][x], grid[y-1][x])):
						if (in_bounds(x-1, y-2) and can_match(grid[y][x], grid[y-2][x-1])):
							moves[(x-1, y-2, x, y-2)].append(grid[y][x])
						if (in_bounds(x, y-3) and can_match(grid[y][x], grid[y-3][x])):
		...
		moves = sorted(moves, key=lambda move: (len(moves[move]) + (.5 if ("5" in moves[move] or "s" in moves[move]) else 0)), reverse = True)
		print(moves)
		finalMove = moves[0]
